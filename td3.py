...
class PolicyNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size, action_range=1., init_w=3e-3, log_std_min=-20, log_std_max=1):
        super(PolicyNetwork, self).__init__()
        
        self.log_std_min = log_std_min
        self.log_std_max = log_std_max

        #print(f"action_range:{action_range}, logstdmin:{log_std_min}, logstdmax:{log_std_max}")
    ...
    def get_action(self, state, deterministic, explore_noise_scale):
        ...
        # print(f"action:{action}+noise:{noise.numpy()}")
        action = np.clip(self.action_range*action + noise.numpy(), -1, 1)
        return action


    def sample_action(self):
        a = Normal(0, 0.2).sample((2,))
...
max_episodes  = 1000
max_steps   = 20 if ENV ==  'Reacher' else 1000  # Pendulum needs 150 steps per episode to learn well, cannot handle 20
frame_idx   = 0
batch_size  = 32
explore_steps = 5000  # for random action sampling in the beginning of training
update_itr = 1
hidden_dim = 300
policy_target_update_interval = 1 # delayed update for the policy network and target networks
DETERMINISTIC=True  # DDPG: deterministic policy gradient
explore_noise_scale = .3  # 0.5 noise is required for Pendulum-v0, 0.1 noise for HalfCheetah-v2
...
class TD3_Trainer():
    ...
    def update(self, batch_size, deterministic, eval_noise_scale, reward_scale=10., gamma=0.99, soft_tau=1e-2):
